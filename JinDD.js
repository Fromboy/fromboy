...
async function main() {
    ...
    for (const item of JinDD) {
        ...
        console.log("————————————")
        console.log("开始任务")
        let taskList = await commonGet('route=plugin.qmtask.api.qmtask.index&')
        for (const task of taskList.data) {
            console.log(`任务：${task.name}`)
            if (task.show_status == 1) {
                console.log('任务已完成')
            } else {
                for (let i = task.has_count; i < task.num; i++) {
                    let finish = await commonGet(`route=plugin.qmtask.api.qmtask.confirm_qmtask`)
...
async function getCookie() {
    ...
    const id = body.data.info.uid;
    const nickname = body.data.info.nickname;
    const newData = {"id": id, "cookie": cookie, "nickname": nickname};
    const index = JinDD.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (JinDD[index].cookie == newData.cookie) {
            return
        } else {
            JinDD[index] = newData;
...
async function commonGet(url) {
    return new Promise(resolve => {
        const options = {
            url: `https://www.jindd.shop/addons/yun_shop/api.php?i=12&uuid=0&type=1&mid=${id}&version=v1.1.137&validate_page=1&${url}`,
            headers : {
